...
sub profiled_element {
  my ($body) = @_;
  my $counters = [0, 0.0];
  gen('profiled', {counters => $counters},
    q{ ++$%counters[0];
       $%counters[1] -= time;
       %<<body
       $%counters[1] += time; }) % {body => $body};
}
...
sub alternatives {
  ...
  for (my $i = $#options; $i >= 0; --$i) {
    $decision = gen("alternative_branch:$i", {r => $r, m => $multiples[$i]},
                  q{ if ($%r < %m) {
                       %<<option
                     } else {
                       %<<next
  ...
  my $alternative    =
    gen($alternative_id, {r => $r, counters => $counters, refinement_data => 0},
